...
export function ActivityTimeline({ processedEvents, isLoading }: ActivityTimelineProps) {
	...
	const getEventIcon = (title: string, index: number) => {
		...
		} else if (title.toLowerCase().includes('companies')) {
			return <Loader2 className='h-4 w-4 text-neutral-400' />;
		} else if (title.toLowerCase().includes('finding')) {
			return <Pen className='h-4 w-4 text-neutral-400' />;
		} else if (title.toLowerCase().includes('curating')) {
			return <Loader2 className='h-4 w-4 text-neutral-400' />;
		}
